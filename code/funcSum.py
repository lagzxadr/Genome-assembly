...
def excludeLowPE(rawData, PEreadsShreshold = 2):
    '''this function exclude BP with <3 PEreads and 0 splitreads'''
...
#        print hgSeq
        ...
        alnSeq = [hbvSeqName + ' ' + 'exp_seq' + ' ' + hgSeq[0], \
            ...
            int(rawData[i][7]), int(rawData[i][8])]
...
def shortSeqFilter(seqResult, flankingLen = 100):
    ...
    excludeList = []
    threshold = 0.6 # filter the inte with M over 60%
    for i in range(len(seqResult)):
        if seqResult[i][2][:flankingLen].count('M') > threshold * flankingLen or \
        seqResult[i][2][-1*flankingLen:].count('M') > threshold * flankingLen or \
        seqResult[i][2][:flankingLen].count('N') > threshold * flankingLen or \
...
#        ha = 'center', va = 'bottom')
    for i in range(N):
        ax.text(p2[i].get_x()+0.5*width,labelheight[i]+7, 'p = %2.1e' % resultP[1][i],
...
def Barplot_3samples_InteC(data, resultP, figureName):
    ...
    for i in range(N):
        ax.text(p2[i].get_x()+0.5*width,labelheight[i]+20, 'p = %2.1e' % resultP[1][i],
        ha = 'center', va = 'bottom',fontsize = 7)        
    for i in range(N):
        ax.text(p3[i].get_x(),labelheight[i]+5, 'p = %2.1e' % resultP[2][i],
        ha = 'center', va = 'bottom', fontsize = 7) 
...
def writeColorTxt(colorList, txtPrefix = ''):
    with open(txtPrefix+'blueList.txt','w') as bluef:
        for blue in colorList[0]:
            bluef.writelines(str(blue)+'\n')
    with open(txtPrefix+'redList.txt','w') as redf:
        for red in colorList[1]:
            redf.writelines(str(red)+'\n')
    with open(txtPrefix+'blackList.txt', 'w') as greyf:
        for grey in colorList[2]:
            greyf.writelines(str(grey)+'\n')
    with open(txtPrefix+'greyList.txt', 'w') as greyf:
...
def setExcelColor(muFileTxt, mhFileTxt, mFileTxt, nFileTxt, flankLen = 100):
    ...
    wb = xl.Workbooks.Open(r'D:\dataFolder\HBV\mhData\hccVISP_003977.v2015.12.29\88sampleSeq.02012016.xlsx')
...
#setExcelColor( 'all_blueList.txt','all_redList.txt','all_blackList.txt','all_greyList.txt',100 )
