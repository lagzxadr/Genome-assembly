...
def plotPairCountvsDiff(diffs,pairCountRes, prefix = ''):
...
def shuffleLibName(rawData):
    ...
    shuffledRawData = []
    for data in rawData:
        otherContent.append(data[:9])
        libName.append([data[9]])    
    # shuffle
    shuffle(otherContent)
    shuffle(libName)
    # recombine
    for (otherc, libN) in zip(otherContent, libName):
        otherc.extend(libN)
        shuffledRawData.append(otherc)  
    return shuffledRawData
...
#    print data
    ...
    for i in range(N-1):
        for j in range(i+1,N):
            P = []
            o = [data[i], totalCount - data[i]]
            e = [data[j], totalCount - data[j]]
...
plot = False
shuffleTimes = 500
diffs = [700] # set the diff on human genome
...
#rawData_filtered = fs.excludeLowPE_v2(rawData, 1)  # 819 integrations
rawData_filtered, seqResult_filtered = fs.theBigFilter(HBV_refName, hg_refName, rawData, 100)  #375 integrations
...
# shuffle the libName
pairCountShuffle = []
for _ in range(shuffleTimes):
    shuffledData = shuffleLibName(rawData_filtered)
    shuffledData_Sorted = sorted(shuffledData, key = itemgetter(2,9,7,8), reverse = False)
    _,pairCount = finding_pair(shuffledData_Sorted, 700) #diff = 700
    pairCountShuffle.append(pairCount)
print pairCountShuffle
expectedPair = sum(pairCountShuffle)/float(len(pairCountShuffle))
print expectedPair, 'pairs are expected.'
    
###############################################################################
#print 'no diff limitation on human DNA'       
#sort the rawData based on chr# and sampleID 2-chr 9-lib# 7-PEr 8-SPr
...
#chisquare()
resultP = chisquareTest([pairCount, expectedPair], 375/2)
print resultP
