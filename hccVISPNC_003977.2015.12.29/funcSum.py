...
def writeExcelBPList(rawData, csvName):
    ...
    with open(csvName + '.csv','wb') as outF:
        ...
        for dataEntry in rawData:
            ...
            dataEntry[4]]])
    with open(csvName+'temp.csv', 'wb') as tempF:
...
#        print rawData[i]
        ...
        if len(rawData[i][5])<extractLen: #hbv length <100
            ...
            hbvLength = len(rawData[i][5])
        else:
            expHbvSeq = rawData[i][5][-extractLen:]
            hbvLength = extractLen
...
#        print hgSeq
        ...
        alnSeq = [hbvSeqName + ' ' + 'exp_seq' + ' ' + hgSeq[0], \
            ...
            ('.').join(rawData[i][:5]), hbvLength,hgLength]
        result.append(alnSeq)
#        result[i]:
#        [[0.hbvseqID,'exp_seq',hgseqID], [1.hbvRefSeq], [2.expSeq],[3.hgRefSeq],\
#        [4.PEreads],[5.Splitreads],[6.sampleID],\
#        [7.'hbvstrand.hbvpos.hgchr.hgstrand.hgpos'],[8.hbvLength],[9.hgLength]]
    return result
...
def theSmallFilter(HBV_refName, hg_refName, rawData, flankLen = 100):
    ...
    print 'after removing PE=1 integrations, {} are left.'.format(len(rawData))    
    ...
    print 'after removing short seq, {} are left.'.format(len(rawData1))
    ...
    print 'after removing high mutation seq, {} are left'.format(len(rawData2))  
...
def theBigFilter(HBV_refName, hg_refName, rawData, flankLen = 100):
    ...
    print 'after removing repeated integrations, {} are left.'.format(len(rawData))
    rawData = excludeLowPE(rawData, 1) # pls change the threshold
    print 'after removing PE=1 SP=0 integrations, {} are left.'.format(len(rawData))
    #extract the sequence
    seqResult =  extractRefSeq(HBV_refName, hg_refName, rawData, flankLen)
    #step1-> filter short exp seq
    excludeList1 = shortSeqFilter(seqResult, flankLen) 
    ...
    print 'after removing high mh seq, {} are left'.format(len(rawData3))
    ...
    print 'after removing high N seq, {} are left'.format(len(rawData_filtered))     
    return rawData_filtered, seqResult_filtered
